def coh_function():
    ...
    with fp.open(mode="r", encoding="UTF-8", newline="") as file:
        ...
        # create an empty list for cash on hand data
        coh=[]
 
        # append cash on hand data to the empty list
        for row in reader:
    ...
    # for loop used to compare Cash on Hand in the current day with the previous day and repeat for all days
    for i in range(len(coh)):
        # include a 0 for Cash on Hand difference in day 11
        if i == 0:
            coh[i].append(0)
        else:
            diff = coh[i][1] - coh[i - 1][1]
            # append differences across all days to the Cash on Hand data
            coh[i].append(diff)
    # print(coh)
